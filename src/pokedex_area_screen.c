...
// NHoenn
#define MAP_GROUP_NHOENN_TOWNS_AND_ROUTES MAP_GROUP(THE_UNDER)
#define MAP_GROUP_NHOENN_DUNGEONS MAP_GROUP(ARTISAN_CAVE_B1F)
    ...
    for (i = 0; sFeebasData[i][0] != NUM_SPECIES; i++)
    {
        if (species == sFeebasData[i][0])
        {
            ...
            case MAP_GROUP_TOWNS_AND_ROUTES:
                if(mapNumber == 0) // Hoenn
                    SetAreaHasMon(sFeebasData[i][1], sFeebasData[i][2]);
                break;
            case MAP_GROUP_NHOENN_TOWNS_AND_ROUTES:
                if(mapNumber == 1) // NHoenn
                    SetAreaHasMon(sFeebasData[i][1], sFeebasData[i][2]);
                break;
            ...
            case MAP_GROUP_SPECIAL_AREA:
                ...
                break;
            case MAP_GROUP_NHOENN_DUNGEONS:
                if(mapNumber == 1) // NHoenn
                    SetAreaHasMon(sFeebasData[i][1], sFeebasData[i][2]);
                break;  
            }
...
void ShowPokedexAreaScreen(u16 species, u8 *screenSwitchState, u8 mapNum)
{
    u8 taskId;
...
static void Task_ShowPokedexAreaScreen(u8 taskId)
{
    ...
    case 1:
        ...
        //NHoenn
        if(mapNumber == 1)
            LoadPokedexAreaMapGfx_NHoenn(&sPokedexAreaMapTemplate);
...
static void CreateAreaMarkerSprites(void)
{
    ...
    else
    {
        ...
        {
            mapSecId = sPokedexAreaScreen->specialAreaRegionMapSectionIds[i];
            x = 8 * (gRegionMapEntries_NHoenn[mapSecId].x + 1) + 4;
            y = 8 * (gRegionMapEntries_NHoenn[mapSecId].y) + 28;
            x += 4 * (gRegionMapEntries_NHoenn[mapSecId].width - 1);
            y += 4 * (gRegionMapEntries_NHoenn[mapSecId].height - 1);
            spriteId = CreateSprite(&sAreaMarkerSpriteTemplate, x, y, 0); 
        if (spriteId != MAX_SPRITES)
        {
            gSprites[spriteId].invisible = TRUE;
