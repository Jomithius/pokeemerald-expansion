...
static void RecordedPlayerHandleIntroTrainerBallThrow(u32 battler)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_RECORDED_LINK)
        trainerPicId = gLinkPlayers[GetBattlerMultiplayerId(battler)].gender + TRAINER_BACK_PIC_LUNA_MALE;
    else
        trainerPicId = gSaveBlock2Ptr->playerGender + TRAINER_BACK_PIC_LUNA_MALE;
