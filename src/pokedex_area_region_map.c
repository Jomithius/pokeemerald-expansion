...
static const u16 ALIGNED(4) sPokedexAreaMap_Pal_NHoenn[] = INCBIN_U16("graphics/pokedex/nhoenn_map.gbapal");
static const u32 sPokedexAreaMap_Gfx_NHoenn[] = INCBIN_U32("graphics/pokedex/nhoenn_map.8bpp.lz");
static const u32 sPokedexAreaMap_Tilemap_NHoenn[] = INCBIN_U32("graphics/pokedex/nhoenn_map.bin.lz");
...
void LoadPokedexAreaMapGfx_NHoenn(const struct PokedexAreaMapTemplate *template)
{
    u8 mode;
    void * tilemap;
    sPokedexAreaMapBgNum = Alloc(sizeof(sPokedexAreaMapBgNum));
    ...
    {
        SetBgAttribute(template->bg, BG_ATTR_METRIC, 0);
        DecompressAndCopyTileDataToVram(template->bg, sPokedexAreaMap_Gfx_NHoenn, 0, template->offset, 0);
        tilemap = DecompressAndCopyTileDataToVram(template->bg, sPokedexAreaMap_Tilemap_NHoenn, 0, 0, 1);
        AddValToTilemapBuffer(tilemap, template->offset, 32, 32, FALSE); // template->offset is always 0, so this does nothing.
    }
    else
    {
    ...
    ChangeBgY(template->bg, 0, BG_COORD_SET);
    SetBgAttribute(template->bg, BG_ATTR_PALETTEMODE, 1);
    CpuCopy32(sPokedexAreaMap_Pal_NHoenn, &gPlttBufferUnfaded[BG_PLTT_ID(7)], sizeof(sPokedexAreaMap_Pal_NHoenn));
    *sPokedexAreaMapBgNum = template->bg;
}
