...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if (sPartyMenuInternal->numActions < 5 && CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES), MOVE_FLY) && CheckBagHasItem(ITEM_HM_FLY, 1) && FlagGet(FLAG_BADGE06_GET)) 
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 5 + MENU_FIELD_MOVES);
    // If Mon can learn HM05 and action list consists of < 4 moves, add FLASH to action list
    if (sPartyMenuInternal->numActions < 5 && CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES), ITEM_HM05 - ITEM_TM01) && CheckBagHasItem(ITEM_HM_FLASH, 1) && FlagGet(FLAG_BADGE02_GET)) 
...
void Task_TeraShard(u8 taskId)
{
    ...
    case 0:
        // Can't use.
        if (tOldTeraType == tNewTeraType || gSpeciesInfo[GetMonData(&gPlayerParty[tMonId], MON_DATA_SPECIES)].forceTeraType)
        {
            gPartyMenuUseExitCallback = FALSE;
            PlaySE(SE_SELECT);
            DisplayPartyMenuMessage(gText_WontHaveEffect, 1);
        ...
        GetMonNickname(&gPlayerParty[tMonId], gStringVar1);
        CopyItemName(gSpecialVar_ItemId, gStringVar2);
        StringExpandPlaceholders(gStringVar4, askTeraText);
        PlaySE(SE_SELECT);
        DisplayPartyMenuMessage(gStringVar4, 1);
        ScheduleBgCopyTilemapToVram(2);
        tState++;
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    if (!(B_RARE_CANDY_CAP && sInitialLevel >= GetCurrentLevelCap() && !(holdEffectParam == 0))) // was (!(B_RARE_CANDY_CAP && sInitialLevel >= GetCurrentLevelCap())), changed so rare candy works over level cap
    {
