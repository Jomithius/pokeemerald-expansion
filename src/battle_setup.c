...
const struct RematchTrainer gRematchTable[REMATCH_TABLE_ENTRIES] =
{
    ...
    [REMATCH_TATE_AND_LIZA] = REMATCH(TRAINER_TATE_AND_LIZA_1, TRAINER_ARIA, TRAINER_MOMO, TRAINER_SHADOW_CLONE_MOMO, TRAINER_TATE_AND_LIZA_5, MOSSDEEP_CITY),
...
void BattleSetup_StartLegendaryBattle(void)
{
    ...
    case SPECIES_TYPE_NULL:
        CreateBattleStartTask(B_TRANSITION_BLACKHOLE, MUS_VS_LAKE_GUARDIANS);
        break;
    ...
    case SPECIES_HOOPA_UNBOUND:
        ...
        break;
    case SPECIES_ARCEUS:
        CreateBattleStartTask(B_TRANSITION_BLACKHOLE, MUS_VS_ARCEUS);
        break;
    ...
    case SPECIES_MARSHADOW:
        CreateBattleStartTask(B_TRANSITION_SHRED_SPLIT, MUS_VS_LAKE_GUARDIANS);
        break;
    }
...
static void TryUpdateGymLeaderRematchFromWild(void)
{
    if (GetGameStat(GAME_STAT_WILD_BATTLES) % 60 == 0 && !FlagGet(FLAG_SYS_FINAL_REMATCH) && !FlagGet(FLAG_SYS_BEFORE_ELITE_4_REMATCH)) // edited
        UpdateGymLeaderRematch();
}

static void TryUpdateGymLeaderRematchFromTrainer(void)
{
    if (GetGameStat(GAME_STAT_TRAINER_BATTLES) % 20 == 0 && !FlagGet(FLAG_SYS_FINAL_REMATCH) && !FlagGet(FLAG_SYS_BEFORE_ELITE_4_REMATCH)) // edited
        UpdateGymLeaderRematch();
}
...
void PlayTrainerEncounterMusic(void)
{
        ...
        && TRAINER_BATTLE_PARAM.mode != TRAINER_BATTLE_CONTINUE_SCRIPT_DOUBLE_NO_MUSIC)
    {
        ...
        case TRAINER_ENCOUNTER_MUSIC_KIMONO:
            ...
            break;
        case TRAINER_ENCOUNTER_MUSIC_BIKER:
            music = MUS_RG_ENCOUNTER_ROCKET;
            break;
        case TRAINER_ENCOUNTER_MUSIC_DISCIPLE:
