...
static const u8 sContextMenuItems_NoReg[] = {
    ACTION_USE, ACTION_CANCEL
};
...
{
    GoToBagMenu(ITEMMENULOCATION_FIELD, FlagGet(FLAG_SYS_RECEIVED_TERA_ORB) ? POCKETS_COUNT : POCKETS_COUNT - 1, CB2_ReturnToFieldWithOpenMenu);
}

void CB2_BagMenuFromBattle(void)
{
    if (!InBattlePyramid())
        GoToBagMenu(ITEMMENULOCATION_BATTLE, FlagGet(FLAG_SYS_RECEIVED_TERA_ORB) ? POCKETS_COUNT : POCKETS_COUNT - 1, CB2_SetUpReshowBattleScreenAfterMenu2);
    else
...
{
    GoToBagMenu(ITEMMENULOCATION_SHOP, FlagGet(FLAG_SYS_RECEIVED_TERA_ORB) ? POCKETS_COUNT : POCKETS_COUNT - 1, CB2_ExitSellMenu);
}

void CB2_GoToItemDepositMenu(void)
{
    GoToBagMenu(ITEMMENULOCATION_ITEMPC, FlagGet(FLAG_SYS_RECEIVED_TERA_ORB) ? POCKETS_COUNT : POCKETS_COUNT - 1, CB2_PlayerPCExitBagMenu);
}

void ApprenticeOpenBagMenu(void)
{
    GoToBagMenu(ITEMMENULOCATION_APPRENTICE, FlagGet(FLAG_SYS_RECEIVED_TERA_ORB) ? POCKETS_COUNT : POCKETS_COUNT - 1, CB2_ApprenticeExitBagMenu);
...
{
    GoToBagMenu(ITEMMENULOCATION_FAVOR_LADY, FlagGet(FLAG_SYS_RECEIVED_TERA_ORB) ? POCKETS_COUNT : POCKETS_COUNT - 1, CB2_FavorLadyExitBagMenu);
    gSpecialVar_Result = FALSE;
}

void QuizLadyOpenBagMenu(void)
{
    GoToBagMenu(ITEMMENULOCATION_QUIZ_LADY, FlagGet(FLAG_SYS_RECEIVED_TERA_ORB) ? POCKETS_COUNT : POCKETS_COUNT - 1, CB2_QuizLadyExitBagMenu);
    gSpecialVar_Result = FALSE;
}
...
void GoToBagMenu(u8 location, u8 pocket, void ( *exitCallback)())
{
    ...
    else
    {
        ...
        if (pocket < (FlagGet(FLAG_SYS_RECEIVED_TERA_ORB) ? POCKETS_COUNT : POCKETS_COUNT - 1))
...
{
    if (deltaBagPocketId == MENU_CURSOR_DELTA_RIGHT && *bagPocketId == (FlagGet(FLAG_SYS_RECEIVED_TERA_ORB) ? POCKETS_COUNT - 1 : POCKETS_COUNT - 2))
        *bagPocketId = 0;
    else if (deltaBagPocketId == MENU_CURSOR_DELTA_LEFT && *bagPocketId == 0)
        *bagPocketId = FlagGet(FLAG_SYS_RECEIVED_TERA_ORB) ? POCKETS_COUNT - 1 : POCKETS_COUNT - 2;
    else
    ...
    default:
        ...
        else
        {
            ...
            case KEYITEMS_POCKET:
                ...
                {
                    gBagMenu->contextMenuNumItems = ARRAY_COUNT(sContextMenuItems_NoReg);
                    memcpy(&gBagMenu->contextMenuItemsBuffer, &sContextMenuItems_NoReg, sizeof(sContextMenuItems_NoReg));
                }
                else
                {
...
{
    GoToBagMenu(ITEMMENULOCATION_LAST, FlagGet(FLAG_SYS_RECEIVED_TERA_ORB) ? POCKETS_COUNT : POCKETS_COUNT - 1, NULL);
}
