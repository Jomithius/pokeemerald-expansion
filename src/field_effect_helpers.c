...
// Sprite data for FLDEFF_TALL_GRASS, FLDEFF_LONG_GRASS, FLDEFF_SNOW_LONG_GRASS, and FLDEFF_ULTRA_SPACE_LONG_GRASS
...
u32 FldEff_UltraSpaceTallGrass(void)
{
    s16 x;
    s16 y;
    u8 spriteId;
    ...
    y = gFieldEffectArguments[1];
    SetSpritePosToOffsetMapCoords(&x, &y, 8, 8);
    spriteId = CreateSpriteAtEnd(gFieldEffectObjectTemplatePointers[FLDEFFOBJ_ULTRA_SPACE_TALL_GRASS], x, y, 0);
    if (spriteId != MAX_SPRITES)
    {
        sprite = &gSprites[spriteId];
        sprite->coordOffsetEnabled = TRUE;
...
void UpdateUltraSpaceTallGrassFieldEffect(struct Sprite *sprite)
{
    u8 mapNum;
    u8 mapGroup;
    u8 metatileBehavior;
    ...
    if (TryGetObjectEventIdByLocalIdAndMap(localId, mapNum, mapGroup, &objectEventId)
     || !MetatileBehavior_IsUltraSpaceTallGrass(metatileBehavior)
     || (sprite->sObjectMoved && sprite->animEnded))
    {
        FieldEffectStop(sprite, FLDEFF_ULTRA_SPACE_TALL_GRASS);
    }
    else
    {
        // Check if the object that triggered the effect has moved away
