...
#define B_SMART_WILD_AI_FLAG        (SYSTEM_FLAGS + 0x8A)     // If not 0, you can set this flag in a script to enable smart wild pokemon
#define B_FLAG_NO_BAG_USE           (SYSTEM_FLAGS + 0x24)  // If this flag is set, the ability to use the bag in battle is disabled.
#define B_FLAG_NO_CATCHING          (SYSTEM_FLAGS + 0x8C)     // If this flag is set, the ability to catch wild Pokémon is disabled.
#define B_FLAG_NO_RUNNING           (SYSTEM_FLAGS + 0x8B)     // If this flag is set, the ability to escape from wild battles is disabled. Also makes Roar/Whirlwind and Teleport (under Gen8) fail.
...
#define B_FLAG_TERA_ORB_CHARGED     (SYSTEM_FLAGS + 0x8F)     // If this flag is set, the Tera Orb is charged. It is automatically set upon healing and cleared upon Terastallizing once configured.
...
#define B_FLAG_SLEEP_CLAUSE         (SYSTEM_FLAGS + 0xAD)     // If this flag is set, sleep clause is enabled; if the player / AI has already put a Pokémon on the opponent's side to sleep and it is still sleeping, another one can't be put to sleep. AI requires AI_FLAG_CHECK_BAD_MOVE to understand.
...
#define B_VAR_DIFFICULTY            0x407D  // If not 0, you can use this var to control which difficulty version of a Trainer is loaded. This should be manually set by the developer using Script_SetDifficulty AFTER NewGameInitData has run.
